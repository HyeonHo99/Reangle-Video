...
def generate_camera_poses(num_poses, angle_step, major_radius, minor_radius, camera_motion_mode, inverse=False):
    ...
    if camera_motion_mode == 'left':
        camera_motion_mode = 'horizontal'
        if angle_step < 0:
            angle_step *= -1

    elif camera_motion_mode == 'right':
        camera_motion_mode = 'horizontal'
        if angle_step > 0:
            angle_step *= -1

    elif camera_motion_mode == 'up':
        camera_motion_mode = 'vertical'
        if angle_step > 0:
            angle_step *= -1

    elif camera_motion_mode == 'down':
        camera_motion_mode = 'vertical'
        if angle_step < 0:
            angle_step *= -1

    elif camera_motion_mode == 'zoomin':
        camera_motion_mode = 'zoomin'
        if angle_step > 0:
            angle_step *= -1
...
def save_warped_image(save_path, images_lists, depth_lists, num_frames, degrees_per_frame, major_radius, minor_radius, camera_motion_mode, dilate_iter, is_segm=False):

    num_frames = num_frames
    # src_pose, poses = generate_camera_poses(num_frames, degrees_per_frame, major_radius, minor_radius, camera_motion_mode)
    ...
    K[0,0] = focal; K[1,1] = focal; K[0,2] = float(W)/2; K[1,2] = float(H)/2
    ...
    for i, pose_t in tqdm(enumerate(poses[0:])):
        image = PIL.Image.open(images_lists[i])
        ...
        warped_np = warped_frame2.astype(np.uint8)  # H×W×3
        hole = (mask2 < 0.5).astype(np.uint8)      # 1 for hole, 0 for valid
        hole8 = (hole * 255).astype(np.uint8)      # 255 for hole, 0 for valid

        # (Optional) save hole mask to disk
        mask_img = PIL.Image.fromarray(hole8)
        os.makedirs(os.path.join(save_path, "mask"), exist_ok=True)
        mask_img.save(os.path.join(save_path, "mask", f"{i:04d}.png"))
        mask_list.append(mask_img)
        

        ## infilling ##
        inpainted = cv2.inpaint(warped_np, hole8, inpaintRadius=3, flags=cv2.INPAINT_TELEA)

        result = PIL.Image.fromarray(inpainted)
        os.makedirs(os.path.join(save_path, "warped"), exist_ok=True)
        result.save(os.path.join(save_path, "warped", f"{i:04d}.png"))
        warped_list.append(result)


    # save masks and warped frames to gif each
    export_to_gif(mask_list, output_gif_path=os.path.join(save_path, "mask.gif"), fps=7)
    export_to_gif(warped_list, output_gif_path=os.path.join(save_path, "warped.gif"), fps=7)

    # save masks and warped frames to mp4 each
    export_to_video(mask_list, os.path.join(save_path, "mask.mp4"), fps=7)
    export_to_video(warped_list, os.path.join(save_path, "warped.mp4"), fps=7)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video_folder', type=str, default="data/frog/video", required=True)
    parser.add_argument('--depth_folder', type=str, default="data/frog/depth", required=True)
    parser.add_argument('--output_folder', type=str, default="training-data/frog", required=True)
    parser.add_argument('--num_frames', type=int, default=49, required=True)
    parser.add_argument('--dilate_iter', type=int, default=1)
    parser.add_argument('--deg_pf', type=float, default=-2, help="degree per frame", required=True)
    parser.add_argument('--camera_motion', type=str, help="should be one of [left, right, up, down, zoomin, zoomout]", required=True)
    args = parser.parse_args()

    assert args.camera_motion in ['left', 'right', 'up', 'down', 'zoomin', 'zoomout']
    assert args.deg_pf != 0

    output_folder = os.path.join(
        args.output_folder, f"{args.camera_motion}_{args.deg_pf}"
    )
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    image_lists = sorted([
        os.path.join(args.video_folder, f) for f in os.listdir(args.video_folder) if f.endswith(('.png', '.jpg', '.jpeg'))
    ])
    depth_lists = sorted([
        os.path.join(args.depth_folder, f) for f in os.listdir(args.depth_folder) if f.endswith(('.npy'))
    ])

    num_frames = args.num_frames
    image_lists = image_lists[:num_frames]
    depth_lists = depth_lists[:num_frames]

    save_warped_image(
        save_path=output_folder,
        images_lists=image_lists,
        depth_lists=depth_lists,
        num_frames=num_frames,
        degrees_per_frame=args.deg_pf,
        major_radius=200,
        minor_radius=200,
        camera_motion_mode=args.camera_motion,
        dilate_iter=args.dilate_iter,
    )
    print(f"Warped *image* saved to {output_folder}")


# [usage]
# python warp_video_dcc_infill.py --video_folder data/frog/video --depth_folder data/frog/depth --output_folder training-data/frog --deg_pf 0.2 --camera_motion left --num_frames 49


# Recommended ranges for camera control
# horizontal (-): [-0.2, -0.25, -0.3] -> right
# horizontal (+): [+0.2, +0.25, +0.3] -> left
# vertical   (-): [-0.1, -0.2, -0.25] -> up
# vertical   (+): [+0.1, +0.15, +0.2] -> down
# zoomin     (-): [-0.4, -0.5,  -0.6] -> zoomin
# zoomout    (+): [+0.1, +0.15, +0.2] -> zoomout
