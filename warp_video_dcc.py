...
def save_warped_image(save_path, images_lists, depth_lists, num_frames, degrees_per_frame, major_radius, minor_radius, camera_motion_mode, dilate_iter):
    ...
    for i, pose_t in tqdm(enumerate(poses[0:])):
        ...
        mask_erosion = cv2.dilate(np.array(mask), kernel, iterations=dilate_iter)
        ...
        mask_erosion.save(os.path.join(save_path, "mask", str(i).zfill(4)+".png"))
        mask_list.append(mask_erosion)
        ...
        warped_frame2.save(os.path.join(save_path, "warped", str(i).zfill(4)+".png"))
        warped_list.append(warped_frame2)
        


    # save masks and warped frames to gif
    ...
    # save masks and warped frames to mp4
