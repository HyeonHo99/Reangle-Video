...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.load_tensors:
            ...
            if os.path.isdir(mask_path):
                visibility_mask = [ 
                    pil_to_tensor(load_image(os.path.join(mask_path, p))).unsqueeze(0) // 255 for p in sorted(os.listdir(mask_path)) 
                ]
                visibility_mask = torch.cat(visibility_mask, dim=0)[:, 0:1, ...].unsqueeze(0)   # shape: [B, F, 1, H, W]
                visibility_mask = 1 - visibility_mask   # 1: visible, 0: invisible
            else:
    ...
    def _load_dataset_from_local_path(self) -> Tuple[List[str], List[str]]:
        ...
        ## load visibility_masks ##
        if self.mask_column is not None:
            mask_path = self.data_root.joinpath(self.mask_column)
        ...
        if self.mask_column is not None:
            with open(mask_path, "r", encoding="utf-8") as file:
                mask_paths = [self.data_root.joinpath(line.strip()) for line in file.readlines() if len(line.strip()) > 0]
        else:
            mask_paths = None
        ...
        return prompts, video_paths, mask_paths
