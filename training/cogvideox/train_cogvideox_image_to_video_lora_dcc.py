...
def log_validation(
    ...
    for _ in range(args.num_validation_videos):
        ...
        video = pipe(**pipeline_args, generator=generator).frames[0]
        videos.append(video)
...
def run_validation(
    ...
    validation_images_dir = os.path.join(args.data_root, args.validation_images_dir)
    validation_images_list = [os.path.join(validation_images_dir, filename) for filename in os.listdir(validation_images_dir)]
    columns_for_grid = []
    # for validation_image, validation_prompt in zip(validation_images, validation_prompts):
    for validation_image in validation_images_list:
        ###############################################
        ### make id_token distinguish camera control ##
        ###############################################
        filename = os.path.basename(validation_image)
        if "right" in filename:
            id_token = "CAMERA_ORBIT_RIGHT" + " " + "Camera movement orbit right,"
        elif "left" in filename:
            id_token = "CAMERA_ORBIT_LEFT" + " " + "Camera movement orbit left,"
        elif "up" in filename:
            id_token = "CAMERA_ORBIT_UP" + " " + "Camera movement orbit up,"
        elif "down" in filename:
            id_token = "CAMERA_ORBIT_DOWN" + " " + "Camera movement orbit down,"
        elif "zoomin" in filename:
            id_token = "CAMERA_ZOOM_IN" + " " + "Camera movement zoom in,"
        elif "zoomout" in filename:
            id_token = "CAMERA_ZOOM_OUT" + " " + "Camera movement zoom out,"
        else:
            id_token = ""

        print(f"filename: {filename} | id_token: {id_token}")
        validation_prompt = id_token + " " + args.validation_prompt
        ...
        column = log_validation(
            ...
            image_id=os.path.basename(validation_image)[:-4],   # exclude .png, .jpg, ...
...
class CollateFunction:
    ...
    def __call__(self, data: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        # visibility_mask #
        visibility_mask = [x["visibility_mask"] for x in data[0]][0]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if batch["visibility_mask"] is not None:
                    visibility_mask = batch["visibility_mask"].to(accelerator.device, non_blocking=True)
                else:
        ...
        if args.validation_prompt and args.num_validation_videos > 0:
            ...
            for validation_image in validation_images_list:
                ...
                video = log_validation(
                    ...
                    image_id=os.path.basename(validation_image)[:-4],
                )
